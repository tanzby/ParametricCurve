...
{
    template <typename F>
    static double simpson_1_3(F&& derivative_func, const double& L, const double& R)
    {
        double mid = (L + R) / 2.0;
    ...
    template <typename F>
    static double adaptive_simpson_1_3(F&& derivative_func, const double& L, const double& R, const double& eps = 0.0001)
    {
    ...
    template <typename F>
    static double simpson_3_8(F&& derivative_func, const double& L, const double& R)
    {
        double mid_L = (2*L + R) / 3.0, mid_R = (L + 2*R) / 3.0;
        return (derivative_func(L) + 3.0 * derivative_func(mid_L) + 3.0 * derivative_func(mid_R) + derivative_func(R)) * (R - L) / 8.0;
    }

    template <typename F>
    static double adaptive_simpson_3_8(F&& derivative_func, const double& L, const double& R, const double& eps = 0.0001)
    {
        const double mid = (L + R) / 2.0;
