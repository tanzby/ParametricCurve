...
class BezierFittingDemo {
  ...
  void static onMouseCallback(int event, int x, int y, int flags, void* userdata) {
    ...
    BezierFittingDemo* demo = static_cast<BezierFittingDemo*>(userdata);
  ...
  void static onTrackerCallback(int pos, void* userdata) {
    BezierFittingDemo* demo = static_cast<BezierFittingDemo*>(userdata);
    demo->curve.fit(demo->points, max(static_cast<double>(demo->max_error), 0.001));
 ...
 public:
  void run() {
    ...
    createTrackbar(
        "max_error", window_name, &max_error, 5000, &BezierFittingDemo::onTrackerCallback, this);
    ...
    while ((key = waitKey(10)) != 'q') {
      ...
      for (const auto& b : curve.getPiecewiseBeziers()) {
        ...
        polylines(img,
                  draw_points,
                  false,
                  Scalar(int(b.length()) * 134214 % 255,
                         int(b.length()) * 523622 % 255,
                         int(b.length()) * 357326 % 255),
                  2,
                  cv::LINE_AA);
      }
      printHelp();
  ...
  void printHelp() {
    ...
    help << "\nThis is a demo for showing Bezier Curve Fitting\n"
         ...
         << "2. Double click the left mouse button quickly to cancel the "
            "previous point\n"
         << "3. Press 'f' to fit a piecewise bezier curve within the specified "
            "error threshold\n"
